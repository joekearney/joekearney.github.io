...
Scala has a built-in implementation of the singleton pattern, in the form of the `object`
...
We're going to use a trivial example to look at some bytecode. This example `ScalaConstants` contains a constant value and a utility function. Below it is the bytecode as shown by `javap -p ScalaConstants`, just the type signatures with no disassembled code, for now.

Notice first that there are two classes, one with a `$` appended to the name. This is synthetically generated by the Scala compiler, and is how the singleton object is implemented.
